...
class RequestManager {
  ...
  getMethod = async (requestConfig, params) => {
    return await new Promise((resolve, reject) => {
      this.axios.get(requestConfig.url, { params }, {
  ...
  postMethod = async (requestConfig, params) => {
    return await new Promise((resolve, reject) => {
      this.axios.post(requestConfig.url, params, {
        headers: requestConfig?.headers || {}
      }).then((res) => {
        if (this.isSuccessRequest(res)) return resolve(res.data);
        else reject(res)
  ...
  generateRequestMethods() {
    const requestKeys = Object.keys(this?.config || {}).filter((key) => key !== "baseUrl");
    const methods = {
      get: this.getMethod,
    ...
    requestKeys.forEach((requestKey) => {
      ...
      this.request[requestKey] = (params) => methods[request.type](request, params)
    });
